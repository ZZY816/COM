...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.part = self.dataset_cfg.get('PART', True)
        self.training = training
        self.class_names = class_names
        self.logger = logger
        data_root1 = 'your dataset'
        root_candidate = [self.dataset_cfg.DATA_PATH, data_root1]
        ...
        self.data_processor = DataProcessor(
            self.dataset_cfg, self.dataset_cfg.DATA_PROCESSOR, point_cloud_range=self.point_cloud_range,
    ...
    def prepare_data(self, data_dict):
        ...
        Returns:
            data_dict:
                ...
                ...
        """
        #print('start', len(data_dict['gt_boxes']))
        ...
        if self.training:
            ...
            if self.part and gt_boxes_mask.sum() == 0:
                #print('new_idx')
                new_index = np.random.randint(self.__len__())
                return self.__getitem__(new_index)
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            if not self.dataset_cfg.get('DISABLE_NPGT', True):
            ...
            if data_dict['occupancy_ratio'] is not None:
                data_dict['occupancy_ratio'] = data_dict['occupancy_ratio'][selected]
            if data_dict['facade_type'] is not None:
                data_dict['facade_type'] = data_dict['facade_type'][selected]
        ...
        #print('end', len(data_dict['gt_boxes']))
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['occupancy_ratio']:
                    max_num = max([x.shape[0] for x in val])
                    occupancy_ratio = np.zeros((batch_size, max_num))
                    for k in range(batch_size):
                        occupancy_ratio[k, :val[k].__len__()] = val[k]

                    ret[key] = occupancy_ratio

                elif key in ['facade_type']:
                    max_num = max([x.shape[0] for x in val])
                    facade_type = np.zeros((batch_size, max_num))
                    for k in range(batch_size):
                        facade_type[k, :val[k].__len__()] = val[k]
