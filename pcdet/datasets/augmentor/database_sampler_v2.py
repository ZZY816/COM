...
class DataBaseSampler(object):
    ...
    def split_groups(self, db_infos, class_name, sample_num):  # important
        ...
        distance_indices = np.array([np.sqrt(np.power(sample['box3d_lidar'][0], 2) +
                                             np.power(sample['box3d_lidar'][1], 2)) for sample in class_dict])
        volume_indices = np.array([sample['box3d_lidar'][3]*sample['box3d_lidar'][4]*sample['box3d_lidar'][5]
                                   for sample in class_dict])
        density_indices = num_points_indices/volume_indices




        easy_density = density_indices[distance_indices <= 30]
        medium_density = density_indices[(distance_indices <= 50) & (distance_indices > 30)]
        hard_density = density_indices[distance_indices > 50]

        easy_sort = easy_density[np.argsort(easy_density)]
        medium_sort = medium_density[np.argsort(medium_density)]
        hard_sort = hard_density[np.argsort(hard_density)]

        easy_threshold = easy_sort[int(len(easy_sort) * 1 / 3)]
        medium_threshold = medium_sort[int(len(medium_sort) * 1 / 3)]
        hard_threshold = hard_sort[int(len(hard_sort) * 1 / 3)]

        easy_indices = np.where((distance_indices <= 30) & (density_indices > easy_threshold))[0]
        medium_indices = np.where((distance_indices <= 50) & (distance_indices > 30) &
                                  (density_indices > medium_threshold))[0]
        hard_indices = np.where((distance_indices > 50) & (density_indices > hard_threshold))[0]


        indices_list = [easy_indices, medium_indices, hard_indices]
        pointer_list = [len(easy_indices), len(medium_indices), len(hard_indices)]
        print(pointer_list, class_name)
    ...
    def sample_with_fixed_number_v2(self, class_name, sample_group, sample_num_list=None):
        ...
        sample_num_list = np.ones(len(pointer_list))/len(pointer_list)
        sampled_dict = []
        for i in range(len(pointer_list)):

            sample_num = int(sample_num_list[i] * total_sample_num) + 1
            if sample_num == 0:
                continue
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        gt_occupancy_ratio = data_dict['occupancy_ratio'][gt_boxes_mask]
        gt_facade_type = data_dict['facade_type'][gt_boxes_mask]
        ...
        sampled_occupancy_ratio = np.array([x['occupancy_ratio'] for x in total_valid_sampled_dict])
        sampled_facade_type = np.array([x['facade_type'] for x in total_valid_sampled_dict])
        assert sampled_occupancy_ratio.shape == sampled_gt_npgt.shape
        assert sampled_facade_type.shape == sampled_gt_npgt.shape
        ...
        gt_occupancy_ratio = np.concatenate([gt_occupancy_ratio, sampled_occupancy_ratio], axis=0)

        gt_facade_type = np.concatenate([gt_facade_type, sampled_facade_type], axis=0)
        ...
        data_dict['occupancy_ratio'] = gt_occupancy_ratio
        data_dict['facade_type'] = gt_facade_type
