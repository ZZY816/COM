...
from . import augmentor_utils, database_sampler, database_sampler_v2,  database_sampler_curriculum
...
class DataAugmentor(object):
    ...
    def gt_sampling(self, config=None):
        ...
        if self.aug_config['AUG_CONFIG_LIST'][0].get('USE_CURRICULUM_AUG', False):
            if self.aug_config['AUG_CONFIG_LIST'][0].get('V2', True):
                db_sampler = database_sampler_v2.DataBaseSampler(
            ...
            elif self.aug_config['AUG_CONFIG_LIST'][0].get('COM', False):
                db_sampler = database_sampler_curriculum.DataBaseSampler_COM2(
                    root_path=self.root_path,
                    sampler_cfg=config,
                    class_names=self.class_names,
                    logger=self.logger
