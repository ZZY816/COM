...
class WaymoDataset(DatasetTemplate):
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            if mode == 'train':
                info_path = self.data_path / sequence_name / ('%s_short.pkl' % sequence_name)
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            if self.training and self.dataset_cfg.get('FILTER_EMPTY_BOXES_FOR_TRAIN', False):
                ...
                annos['occupancy_ratio'] = annos['occupancy_ratio'][mask]
                annos['facade_type'] = annos['facade_type'][mask]


                if self.training and not self.dataset_cfg.get('DISABLE_TO', True):
            ...
            input_dict.update({
                ...
                'occupancy_ratio': annos.get('occupancy_ratio', None),
                'facade_type': annos.get('facade_type', None)
            })

        data_dict = self.prepare_data(data_dict=input_dict)
        data_dict['metadata'] = info.get('metadata', info['frame_id'])

        if self.training and not self.dataset_cfg.get('DISABLE_NPGT', True):
            assert len(data_dict['gt_boxes']) == len(data_dict['num_points_in_gt'])
        if self.training and not self.dataset_cfg.get('DISABLE_TO', True):
            assert len(data_dict['gt_boxes']) == len(data_dict['true_object'])

        if self.training:
            assert len(data_dict['num_points_in_gt']) == len(data_dict['facade_type'])
            assert len(data_dict['num_points_in_gt']) == len(data_dict['occupancy_ratio'])
        ...
        if not self.training or self.dataset_cfg.get('DISABLE_TO', True):
            data_dict.pop('true_object', None)
            data_dict.pop('occupancy_ratio', None)
            data_dict.pop('facade_type', None)
    ...
    def create_gt_database_of_single_scene(self, info_with_idx, database_save_path=None, use_sequence_data=False, used_classes=None,
        ...
        if info_idx % 1 != 0 and len(names) > 0:
            mask = (names == 'Vehicle')
            names = names[~mask]
            difficulty = difficulty[~mask]
            gt_boxes = gt_boxes[~mask]
