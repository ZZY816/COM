...
class CurriculumCenterHead(nn.Module):
    ...
    def group_classifier(self, x, y, dx, dy, dz, yaw, num_points_in_gt, true_object, class_id):
    ...
    def assign_target_of_single_head(
            ...
            gaussian_overlap=0.1, min_radius=2, npgt=None, true_object=None
    ):
        """
        Args:
            gt_boxes: (N, 8)
        ...
        if true_object is not None:
            radius_map = gt_boxes.new_zeros(num_max_objs, 5).long() # class, center_x, center_y, radius, group
        else:
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            radius_map[k, 1] = int(center[k][0])
            radius_map[k, 2] = int(center[k][1])
            radius_map[k, 3] = radius[k]
            if true_object is not None:
                # group = self.group_classifier(x=x[k], y=y[k], dx=gt_boxes[:, 3][k], dy=gt_boxes[:, 4][k],
                #                               dz=gt_boxes[:, 5][k], yaw=gt_boxes[k, 6],
                #                               num_points_in_gt=num_points_in_gt, true_object=int(true_object[k]),
                #                               class_id=cur_class_id)
                radius_map[k, 4] = true_object[k]



        return heatmap, ret_boxes, inds, mask, radius_map, heatmap_mask

    def assign_targets(self, gt_boxes, feature_map_size=None, npgt=None, true_object=None, **kwargs):
        """
        Args:
            gt_boxes: (B, M, 8)
            npgt: (B, M)
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                if true_object is not None:
                    cur_true_object = true_object[bs_idx]
                ...
                for idx, name in enumerate(gt_class_names):
                    ...
                    if true_object is not None:
                        temp_to = cur_true_object[idx]
                        to_single_head.append(temp_to)
                ...
                if len(gt_boxes_single_head) == 0:
                    ...
                    if true_object is not None:
                        to_single_head = cur_true_object[:0]

                else:
                    gt_boxes_single_head = torch.cat(gt_boxes_single_head, dim=0)
                    npgt_single_head = torch.tensor(npgt_single_head)
                    if true_object is not None:
                        to_single_head = torch.tensor(to_single_head)
                ...
                heatmap, ret_boxes, inds, mask, radius_map, heatmap_mask = self.assign_target_of_single_head(
                    ...
                    true_object=to_single_head.cpu() if true_object is not None else None
                )
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            hm_loss, box_mask, avg_confidence, confidence_true, confidence_aug = self.hm_loss_func(pred_dict['hm'], target_dicts['heatmaps'][idx],
            ...
            conf_true += confidence_true
            conf_aug += confidence_aug
            confidence = confidence / len(pred_dicts)
            conf_true = conf_true / len(pred_dicts)
            conf_aug = conf_aug / len(pred_dicts)

        tb_dict['rpn_loss'] = loss.item()
        tb_dict['confidence'] = confidence
        tb_dict['confidence_true'] = conf_true
        tb_dict['confidence_aug'] = conf_aug
        return loss, tb_dict
            ...
            for length_condition in length_condition_list:
                for facade_condition in facade_condition_list:
                    for occupancy_condition in occupancy_car_condition_list:
                        group[distance_condition & length_condition & facade_condition & occupancy_condition & (class_id == 1) & (true_object==1)] = car_group
                        car_group += 1
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            group = self.cluster(gt_boxes=data_dict['gt_boxes'], true_object=data_dict.get('true_object', None),
                                 occupancy_ratio=data_dict['occupancy_ratio'], facade_type=data_dict['facade_type'])
            target_dict = self.assign_targets(
                data_dict['gt_boxes'], feature_map_size=spatial_features_2d.size()[2:],
                feature_map_stride=data_dict.get('spatial_features_2d_strides', None),
                npgt=data_dict['num_points_in_gt'], true_object=group
            )
